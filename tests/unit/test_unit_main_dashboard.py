...
def admin_dashboard(mock_system):
    ...
    with (
        patch("safehome.interface.dashboard.main_dashboard.tk.Toplevel"),
        ...
        patch(
            "safehome.interface.dashboard.main_dashboard.messagebox"
        ) as mock_messagebox,
        patch(
            "safehome.interface.dashboard.main_dashboard.simpledialog"
        ) as mock_simpledialog,
    ):

        with patch.object(MainDashboard, "_update_loop", lambda self: None):
            dash = MainDashboard(
                system=mock_system, login_window=MagicMock(), user_id="admin"
            )
...
def guest_dashboard(mock_system):
    ...
    with (
        ...
        with patch.object(MainDashboard, "_update_loop", lambda self: None):
            dash = MainDashboard(
                system=mock_system, login_window=MagicMock(), user_id="guest"
            )
            dash.mock_messagebox = mock_messagebox
            dash.mock_simpledialog = mock_simpledialog
            dash.user_id = "guest"
...
def test_dashboard_permission_denied_actions(guest_dashboard):
    ...
    actions = [
        (
            guest_dashboard._set_mode,
            (SafeHomeMode.AWAY,),
            "Guest account cannot change system mode.",
        ),
        (
            guest_dashboard._trigger_panic,
            (),
            "Guest account cannot trigger panic alarm.",
        ),
        ...
        (
            guest_dashboard._open_log_viewer,
            (),
            "Guest account cannot view system logs.",
        ),
        (
            guest_dashboard._open_sensor_simulator,
            (),
            "Guest account cannot open the sensor simulator.",
        ),
        (
            guest_dashboard._toggle_camera,
            (MagicMock(), True),
            "Guest users do not have permission to change camera status.",
        ),
        (
            guest_dashboard._set_camera_password,
            (MagicMock(),),
            "Only admin can change camera passwords.",
        ),
        (
            guest_dashboard._delete_camera_password,
            (MagicMock(),),
            "Only admin can delete camera passwords.",
        ),
    ]
    ...
    for action, args, expected_msg in actions:
        ...
        try:
            ...
            raise AssertionError(
                f"Action '{action.__name__}' failed assertion. Expected msg: '{expected_msg}', Got: {guest_dashboard.mock_messagebox.showwarning.call_args}"
            ) from e
...
def test_set_mode_admin(admin_dashboard):
    ...
    admin_dashboard.mock_messagebox.showinfo.assert_called_with(
        "Success", "System Disarmed"
    )
    ...
    admin_dashboard.mock_messagebox.showinfo.assert_called_with(
        "Success", "System Armed in AWAY mode"
    )
    ...
    admin_dashboard.mock_messagebox.showwarning.assert_called_with(
