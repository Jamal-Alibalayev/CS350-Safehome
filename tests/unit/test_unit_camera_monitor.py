...
class TestCameraMonitor(unittest.TestCase):
    ...
    def test_init_success(self, mock_imagetk, mock_messagebox):
        ...
        monitor = CameraMonitor(
            master=self.master, system=self.mock_system, camera_id=1, password=None
        )
    ...
    def test_init_camera_not_found(self, mock_messagebox):
        ...
        monitor = CameraMonitor(
            master=self.master, system=self.mock_system, camera_id=1
        )
    ...
    def test_init_invalid_password(self, mock_messagebox):
        ...
        monitor = CameraMonitor(
            master=self.master, system=self.mock_system, camera_id=1, password="wrong"
        )

        mock_messagebox.showerror.assert_called_with(
            "Access Denied", "Invalid camera password"
        )
    ...
    def test_update_feed_unavailable(self, mock_messagebox):
        ...
        self.mock_system.camera_controller.get_camera_view.assert_called_with(1, None)
        monitor.image_label.config.assert_called_with(
            text="Camera Unavailable", fg="red"
        )
        monitor.after.assert_called_with(100, monitor._update_feed)
        monitor.destroy()
    ...
    def test_ptz_controls(self, mock_messagebox):
        ...
        monitor = CameraMonitor(
            master=self.master, system=self.mock_system, camera_id=1, password="ok"
        )

        # Pan Left
        self.mock_system.camera_controller.pan_camera.return_value = True
        monitor._pan_left()
        self.mock_system.camera_controller.pan_camera.assert_called_with(
            1, "left", "ok"
        )
        ...
        mock_messagebox.showwarning.assert_called_with(
            "Pan Failed", "Cannot pan left - limit reached or access denied"
        )
        ...
        monitor._pan_right()
        mock_messagebox.showwarning.assert_called_with(
            "Pan Failed", "Cannot pan right - limit reached or access denied"
        )
        ...
        monitor._zoom_in()
        mock_messagebox.showwarning.assert_called_with(
            "Zoom Failed", "Cannot zoom in - limit reached or access denied"
        )

        # Zoom Out
        self.mock_system.camera_controller.zoom_camera.return_value = True
        monitor._zoom_out()
        self.mock_system.camera_controller.zoom_camera.assert_called_with(
            1, "out", "ok"
        )
        ...
        monitor._zoom_out()
        mock_messagebox.showwarning.assert_called_with(
            "Zoom Failed", "Cannot zoom out - limit reached or access denied"
        )
